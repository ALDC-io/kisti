...
class AudioPlayer(QObject):
    ...
    def _generate_and_play(self, text: str, urgency: str = "normal") -> None:
        ...
        try:
            ...
            # PulseAudio keeps HDMI DAC active — no silence prepend needed
            # (was 500ms for direct ALSA where DAC slept between plays)
    ...
    def _expand_abbreviations(text: str) -> str:
        ...
        text = re.sub(r'\bPSI\b', 'pounds', text)
        text = re.sub(r'\bpsi\b', 'pounds', text)
        text = re.sub(r'\bRPM\b', 'R P M', text)
        text = re.sub(r'\bWHP\b', 'wheel horsepower', text)
        text = re.sub(r'\bwhp\b', 'wheel horsepower', text)
        text = re.sub(r'\bBHP\b', 'brake horsepower', text)
        text = re.sub(r'\bbhp\b', 'brake horsepower', text)
        ...
        text = re.sub(r'\bSTI\b', 'S T I', text)
